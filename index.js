...
function indekstekiCesitiGetir(indekstekiCesit, index){
  
  return indekstekiCesit[index];
...
function ismeGoreCesitCikar(liste, cikarilan){
  
  let c = liste.indexOf(cikarilan);
  liste.splice(c, 1);
...
function ismeGoreFiltrele(yeniDizi, filtre){

  let x = [];

  for (let i = 0; i < yeniDizi.length; i++) {

    if(yeniDizi[i].includes(filtre)){
...
function listeSecimi(){

    let liste = Math.floor(Math.random() * 3);
...
function rastgeleTatlar(){
  ...
  while (rastgeleTatlarListesi.length <= 24)
  {
   ...
   let secilenTatlar = listeSecimi();

   let tat = Math.floor(Math.random() * (secilenTatlar.length));

   if(rastgeleTatlarListesi.includes(secilenTatlar[tat]) === false){
 
     rastgeleTatlarListesi.push(secilenTatlar[tat]);
     i++;
   }
